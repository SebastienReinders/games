...
function create() {
  ...
  //on garde un trace du nbre de balles actives dans le jeu
...
function update() {
  ...
  // Gestion des balles (si elle sort de l écran on la désactive)
...
function hitPaddle(ball, paddle) 
{
  ...
  // Inverser la vitesse en Y de la balle pour qu'elle remonte vers le haut
...
function gameOver(scene) {
  ...
  gameOverText = scene.add.text(15, 150, 'Fin du jeu', { fontSize: '32px', fill: '#FFF', fontStyle: 'bold'});
         ...
         var data = {
          action: "MAJscore",
          pseudo: getCookieValue("userLogin"),
           jeu: 'CasseBriques',
           points: score
        };
         $.ajax({
...
function addBrickRow() {
  // Décalez toutes les briques existantes vers le bas pour laisser place a la nouvelle ligne bonus
  ...
  // Ici on itere pour ajouter les briques une par une aux bonnes coordonnées x et y
  for (let j = 0; j < columns; j++) {
    let x = offsetX + j * (brickWidth + brickSpacing);
    let y = offsetY;
...
function getCookieValue(cookieName) {
  // Divise la chaîne de cookies en paires clé-valeur (tableau de clé-valeur)
  var cookiePairs = document.cookie.split(";");

  // Recherche la paire clé-valeur correspondant à la clé spécifiée
  // trim pour effacer les espaces blancs
  var cookiePair = cookiePairs.find(function(pair) {
    return pair.trim().startsWith(cookieName + "=");
  });

  if (cookiePair) {
    // Extrait et renvoie la valeur du cookie
    return cookiePair.split("=")[1].trim();
  }
